...
. "$PARENT_DIR/travis/odb-shared.sh"
...
if [ ! -d "$ODB_DIR" ]; then
  ...
  cp $PARENT_DIR/travis/orientdb-server-config.xml "${ODB_DIR}/config/"
  cp $PARENT_DIR/travis/orientdb-server-log.properties "${ODB_DIR}/config/"
else
